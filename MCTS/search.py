...
for letter in range(16): #Create every node on the map
    ...
    for number in range(16):
        node = Node(str(string.ascii_lowercase[letter]) + str(number + 1), letter, number)
...
for _, node in nodesByName.items():
    print("MapTile " + str(node) + "(\"" + str(node) + "\");") 
    print("addTile(&" + str(node) + ");");
for _, node in nodesByName.items():
    for adjacent in node.getAdjacentVertices():
        print(str(node) + ".addAdjacentTile(&" + str(adjacent) + ");")

def heuristic(vertex, goalVertex): #Returns a heuristic evaluation of a vertex
    distance = (goalVertex.x - vertex.x) + (goalVertex.y - vertex.y)
    liberties = len(vertex.getAdjacentVertices())
    return distance - liberties

def bestFirstSearch(start,finish):
    stack = [start]
    visited = set()
    ...
    while stack:
        ...
        unvisitedVertices.sort(key = lambda v1: heuristic(v1, finish))
...
print(bestFirstSearch(nodesByName["a1"],nodesByName["p16"]))
