...
public class MonteCarloSimulator {
    ...
    public int NUM_ROLLOUTS_PER_ACTION = 1; //how many rollouts we perform on each valid action from the base game state
    ...
    public double rollout(GameState gameState) { //Returns the value of one rollout for an action - 0 is a loss, 1 is a win, 0.5 is a tie.
        ...
        double evaluation = gameState.evaluateTerminalState(MAX_ROLLOUT_DEPTH_TURNS);
        while(evaluation == -1) { //While we haven't reached a terminal state
            ...
            chosenAction.applyActionToGameState(gameState); //Apply the action to the game state
            System.out.println(chosenAction.toString());
            evaluation = gameState.evaluateTerminalState(MAX_ROLLOUT_DEPTH_TURNS);
        }
